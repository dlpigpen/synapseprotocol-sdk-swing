import { isArray } from "lodash-commonjs-es";
...
function useCalculateAddLiquidity(args: {
  ...
  function fn() {
    ...
    if (isArray(amounts)) {
      ...
      amountsArray = lpToken.liquidityAmountsFromMap(
        amounts as SwapPools.PoolTokensAmountsMap
      );
    }
...
function useAddLiquidity(args: {
  ...
  function fn() {
    ...
    TokenSwap.addLiquidity({
      ...
      minToMint: parseBigNumberish(
        rest.minToMint,
        rest.lpToken.baseToken,
        chainId
      ),
    })
      .then(setTx)
...
function useRemoveLiquidity(args: {
  ...
  function fn() {
    ...
    if (isArray(minAmounts)) {
      ...
      minAmountsArray = lpToken.liquidityAmountsFromMap(
        minAmounts as SwapPools.PoolTokensAmountsMap
      );
    }
...
function useSwapTokens(args: {
  ...
  function fn() {
    ...
    TokenSwap.swapTokens(fnParams).then(setTx).catch(logError);
  }
...
function useApproveLPToken(args: {
  ...
  const [approveSpend, approveTx] = useApproveTokenSpend(ethereum, chainId),
  ...
  function fn() {
    const {
      amount,
      lpToken: { swapAddress: spender, baseToken: token },
    } = rest;
...
function useApprovePoolToken(args: {
  ...
  function fn() {
    ...
    Promise.resolve(approveSpend({ token, spender })).then(() => {
      setApproveData({ token, spender, amount });
    });
  }
...
function useLPTokenAllowance(args: {
  ...
  useEffect(() => {
    if (lpToken) {
      const { baseToken: token, swapAddress: spender } = lpToken;
...
function useLPTokenNeedsApproval(args: {
  ...
  const { chainId, amount, lpToken } = args;
...
function useLPTokenApproval(args: {
  ...
  const [execApprove, approveTx, approveStatus] = useApproveLPToken({
    ...args,
    amount: undefined,
  });
...
function usePoolTokenApproval(args: {
  ...
  const [execApprove, approveTx, approveStatus] = useApprovePoolToken({
    ...args,
    amount: undefined,
  });
