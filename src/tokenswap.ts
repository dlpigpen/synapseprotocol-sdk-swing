import { isNil, isNull, isEmpty, isUndefined } from "lodash-commonjs-es";

import { ChainId, ChainIdTypeMap, supportedChainIds } from "@chainid";
...
export namespace UnsupportedSwapErrors {
  ...
  export const tokenNotSupportedNetTo = (
    t: { symbol: string },
    netName: string
  ): UnsupportedSwapError =>
    new UnsupportedSwapError({
  ...
  export const unsupportedMultiJEWELMigration = (): UnsupportedSwapError =>
    new UnsupportedSwapError({
...
export namespace TokenSwap {
  const BRIDGE_CONFIG_INSTANCE: BridgeConfigV3Contract =
  ...
  const ADD_LIQUIDITY_GAS_LIMT: BigNumber = BigNumber.from("210000"),
  ...
  export function bridgeSwapSupported(
    args: BridgeSwapSupportedParams
  ): SwapSupportedResult {
  ...
  export async function calculateAddLiquidity(
    ...
    return swapInstance.calculateTokenAmount(args.amounts, true);
  }
   ...
   */
  export async function calculateRemoveLiquidityOneToken(
    args: RemoveLiquidityOneParams
  ): Promise<BigNumber> {
    const swapInstance = await swapContractFromLPSwapAddress(
      args.lpToken.swapAddress,
      args.chainId
    ...
    if (!tokenAddress) {
      /* c8 ignore start */
      const err = new Error(
        `no address for token ${args.token.name} found for chain id ${args.chainId}`
      );
    ...
    return swapInstance.calculateRemoveLiquidityOneToken(
      args.amount,
      tokenIndex
    );
  }
   ...
   * @return {Promise<ContractTransaction>} Executed transaction object.
   */
  export async function removeLiquidityOneToken(
    args: RemoveLiquidityOneParams
  ): Promise<ContractTransaction> {
    if (isEmpty(args.signer)) {
      /* c8 ignore start */
   ...
   * @return {Promise<PopulatedTransaction>} Populated transaction object.
   */
  export async function buildRemoveLiquidityOneTokenTransaction(
    args: RemoveLiquidityOneParams
  ): Promise<PopulatedTransaction> {
    if (isEmpty(args.deadline)) {
      /* c8 ignore start */
  ...
  function rejectPromiseMissingField(
    fieldName: string,
    paramsType: string
  ): Promise<never> {
  ...
  export async function swapTokens(
    ...
    return resolveSwapData(args)
      .then((swapSetup) => {
        ...
        return swapContract(args.tokenFrom, args.chainId, args.signer).then(
          (swapInstance) => {
  ...
  async function resolveSwapData(
    args: SwapTokensParams | SwapParams
  ): Promise<SwapSetup> {
    const { swapData } = args;
  ...
  function populateSwapTransaction(
    ...
    return txnProm.then((txn) =>
      populateGasOptions(txn, gasOpts, args.chainId)
    );
  }
  ...
  export function intermediateTokens(
    ...
    token: Token,
    otherChainId?: number
  ): IntermediateSwapTokens {
    if (mintBurnSwapTypes.includes(token.swapType)) {
      switch (token.swapType) {
        case SwapType.JEWEL:
          let bridgeConfigIntermediate: Token =
            chainId === ChainId.HARMONY ? Tokens.SYN_JEWEL : Tokens.JEWEL;

          return {
            intermediateToken: Tokens.JEWEL,
            bridgeConfigIntermediateToken: bridgeConfigIntermediate,
          };
        // case SwapType.AVAX:
        //     break;
        default:
    ...
    if (chainId === ChainId.KLAYTN) {
      ...
      if (
        BridgeUtils.isL2ETHChain(otherChainId) &&
        token.swapType === SwapType.ETH
      ) {
        return {
          intermediateToken: Tokens.NETH,
          bridgeConfigIntermediateToken: Tokens.NETH,
        };
      }
    ...
    let intermediateToken: Token, bridgeConfigIntermediateToken: Token;
    ...
    switch (token.swapType) {
      case SwapType.SYN:
        ...
        break;
      case SwapType.FRAX:
        bridgeConfigIntermediateToken =
          chainId === ChainId.ETH ? Tokens.FRAX : Tokens.SYN_FRAX;
        break;
      case SwapType.ETH:
        intermediateToken = Tokens.NETH;
        bridgeConfigIntermediateToken =
          chainId === ChainId.ETH ? Tokens.WETH : Tokens.NETH;
        break;
      case SwapType.AVAX:
        const fromAvax = chainId === ChainId.AVALANCHE,
        ...
        if (!isNil(otherChainId)) {
          if (fromAvax && otherChainId === ChainId.HARMONY) {
            ...
            intermediateToken = Tokens.WAVAX;
            bridgeConfigIntermediateToken =
              chainId === ChainId.HARMONY ? Tokens.SYN_AVAX : Tokens.WAVAX;
            break;
          }
        }
    ...
    bridgeConfigIntermediateToken =
      bridgeConfigIntermediateToken ?? intermediateToken;
      ...
      res[c1] = networkTokens
        .map((t: Token) => {
          ...
          for (const c2 of allChainIds) {
            ...
            if (
              c1 === ChainId.AVALANCHE &&
              t.isEqual(Tokens.MULTIJEWEL) &&
              c2 !== ChainId.DFK
            ) {
              continue;
            }
            ...
            let outToks: Token[] = SwapPools.tokensForChainBySwapGroup(
              c2,
              swapType
            );
  ...
  export async function swapContract(
    token: Token,
    chainId: number,
    signer?: Signer
    ...
    return BRIDGE_CONFIG_INSTANCE.getPoolConfig(
  ...
  async function swapContractFromLPSwapAddress(
  ...
  function _intermediateToken(token: Token, chainId: number): Token {
    const { intermediateToken, bridgeConfigIntermediateToken } =
      intermediateTokens(chainId, token);
    /* c8 ignore next */
    return intermediateToken ?? bridgeConfigIntermediateToken;
  }
  ...
  function checkCanSwap(
    ...
    const tokensCanSwap = checkTokensCanSwap(tokenFrom, tokenTo),
      tokensSupported = checkTokensSupported(
        tokenFrom,
        tokenTo,
        chainFrom,
        chainTo
  ...
  function checkTokensCanSwap(
    tokenFrom: Token,
    tokenTo: Token
  ): SwapSupportedResult {
    let swapSupported: boolean = true,
      reasonNotSupported: UnsupportedSwapErrors.UnsupportedSwapError;
    ...
    if (hasDestChain) {
      if (tokenFrom.isEqual(Tokens.MULTIJEWEL) && chainIdTo !== ChainId.DFK) {
        return {
          swapSupported: false,
          reasonNotSupported:
            UnsupportedSwapErrors.unsupportedMultiJEWELMigration(),
        };
      }
    }
