import { findKey, fromPairs } from "lodash-commonjs-es";
...
const CHAINID_URI_MAP: StringMap = fromPairs(
  supportedChainIds().map((cid) => [cid, getChainRpcUri(cid)])
);
...
export function getChainRpcUri(chainId: number): string {
  const rpcEnvKey: string = ENV_KEY_MAP[chainId],
    rpcEnvVal: string | undefined =
      rpcEnvKey in process.env ? process.env[rpcEnvKey] : undefined;
