import { first, last, sortBy } from "lodash-commonjs-es";
...
export function fetchChainGasPrices(chainId: number): Promise<ChainGasPrices> {
  const supports1559 = chainSupportsEIP1559(chainId),
    provider = rpcProviderForChain(chainId),
    lastPriceProm = provider.getGasPrice();

  return provider.getBlockWithTransactions("latest").then(({ transactions }) =>
    lastPriceProm.then((lastPrice) => {
      const gasPrices: BigNumber[] = extractGasPrices(
        transactions,
        supports1559
      );
...
function extractGasPrices(
  ...
  let gasPrices: BigNumber[] = txns
    ...
    .filter(
      (val) => typeof val !== "undefined" && val !== null && !val.isZero()
    );
