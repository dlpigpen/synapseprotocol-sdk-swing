import { isEqual } from "lodash-commonjs-es";

import { Tokens } from "@tokens";

import { ChainId, supportedChainIds, type ChainIdTypeMap } from "@chainid";
...
import { SwapType, type SwapTypeMap } from "@internal/swaptype";
...
export namespace SwapPools {
  ...
  export class SwapToken implements SwapPoolToken {
    ...
    liquidityAmountsFromMap(m: PoolTokensAmountsMap): BigNumber[] {
      ...
      if (!isEqual(mapKeys, wantMapKeys)) {
        const err = new Error(
          `expected passed PoolTokensAmountsMap to have keys ${wantMapKeys}; got ${mapKeys}`
        );
        console.error(err);
        return amounts;
      }
  ...
  const AVAX_JEWEL_Pool: LPToken = {
      poolTokens: [Tokens.JEWEL, Tokens.MULTIJEWEL],
      swapType: SwapType.JEWEL,
    },
    DFK_JEWEL_Pool: LPToken = {
      poolTokens: [Tokens.GAS_JEWEL, Tokens.JEWEL],
      swapType: SwapType.JEWEL,
    },
    HARMONY_JEWEL_Pool: LPToken = {
      poolTokens: [Tokens.JEWEL, Tokens.SYN_JEWEL],
      swapType: SwapType.JEWEL,
    };
  ...
  export const bridgeSwappableMap: ChainSwapTypePoolsMap = {
    ...
    [ChainId.OPTIMISM]: makeSwapTypeMap(
      {
        usdPool: [
          ...
          OPTIMISM_POOL_SWAP_TOKEN.poolTokensForBridgeSwaps,
        ],
    ...
    [ChainId.CRONOS]: makeSwapTypeMap({
      usdPool: [
        ...
        CRONOS_POOL_SWAP_TOKEN.poolTokensForBridgeSwaps,
      ],
    }),
    ...
    [ChainId.KLAYTN]: {
      ...
      swappableSwapGroups: {
        [SwapType.ETH]: {
          poolTokens: [Tokens.ETH, Tokens.WETH],
          swapType: SwapType.ETH,
        },
  ...
  function checkSwapPoolTokenAddress(
    address: string,
    field: CheckAddressKind,
    chainId?: number
  ): SwapPoolToken {
  ...
  export function swapPoolTokenFromLPTokenAddress(
    lpTokenAddress: string,
    chainId?: number
  ): SwapPoolToken {
    return checkSwapPoolTokenAddress(
      lpTokenAddress,
      CheckAddressKind.LpTokenAddress,
      chainId
    );
  }

  export function swapPoolTokenFromSwapAddress(
    swapAddress: string,
    chainId?: number
  ): SwapPoolToken {
    return checkSwapPoolTokenAddress(
      swapAddress,
      CheckAddressKind.SwapAddress,
      chainId
    );
  }

  export function swapPoolTokenForTypeForChain(
    chainId: number,
    swapType: SwapType
  ): SwapPoolToken {
    let res: SwapPoolToken = null;
